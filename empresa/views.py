...
def nova_empresa(request):
    if request.method == "GET":
        ...
        if (len(nome.strip()) == 0 or len(email.strip()) == 0 or len(cidade.strip()) == 0 or len(endereco.strip()) == 0 or len(nicho.strip()) == 0 or len(caracteristicas.strip()) == 0 or (not logo)):
            ...
            return redirect('/home/nova_empresa') ##Valida se nenhum campo contenha espaço em branco

        if logo.size > 100_000_000: ##Validando se o arquivo enviado é igual a 10 Mbytes
            messages.add_message(request, constants.ERROR, 'A logo da empresa deve ter menos de 10MB')
            return redirect('/home/nova_empresa')

        if nicho not in [i[0] for i in Empresa.choices_nicho_mercado]: ##Valida se o nicho de mercado está presente dentro de empresa
...
def empresa(request, id):
    ...
    empresas = Empresa.objects.all()
    tecnologias = Tecnologias.objects.all()
    vagas = Vagas.objects.filter(empresa__id = id)
